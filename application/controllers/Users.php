...
class Users extends CI_Controller {
	...
	function register(){
		if($this->session->userdata('logged_in'))
		{
			...
			if ($this->User_model->userNameExists($username)) {
				outputJSON(null, false, "<div class='alert alert-danger'><strong>Username ".$username."</strong> already in use.</div>"); die();
			}
			else
			{
				$data['name'] = $name;
				...
				outputJSON(null, true, "<div class='alert alert-success'><strong>User".$username." </strong> registred successfully.</div>"); die();
			}
		}
		else
		{
	...
	function checkUserName(){
		...
		if (!isset($username)) {
			...
			if ($this->User_model->userNameExists($username)) {
				...
				outputJSON(null, true, "<div class='alert alert-success'><strong>Username".$username."</strong> available.</div>"); die();
			}
		}
	}
	...
	function edit_user_db($id=null){
		if($this->session->userdata('logged_in'))
		{
			...
			outputJSON(null, true, "<div class='alert alert-success'><strong>User ".$username."</strong> updated successfully.</div>"); die();
		}
		else
		{
			redirect("users/login", 'refresh');
